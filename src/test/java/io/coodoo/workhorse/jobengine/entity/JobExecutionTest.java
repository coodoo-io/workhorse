...
public class JobExecutionTest {
    ...
    /**
     * Tests that query 'JobExecution.getFirstCreatedByJobIdAndParamters' has not changed since this test had been created. If this test fails, you should consider re-generating ALL methods created from that query as they may be out-dated.
     *
     */
    @SuppressWarnings({"unchecked", "rawtypes", "null"})
    @org.junit.Test 
    public void testGetFirstCreatedByJobIdAndParamtersQueryUnchanged()
    {
    	List annotations = new ArrayList();
    	NamedQuery namedQueryAnnotation = io.coodoo.workhorse.jobengine.entity.JobExecution.class.getAnnotation(NamedQuery.class);
    	if (namedQueryAnnotation == null) {
    	...
    	for (Object obj : annotations) {
    	NamedQuery query = (NamedQuery) obj;
    	if (query.name().equals("JobExecution.getFirstCreatedByJobIdAndParamters")) {
    	queryUnderTest = query;
    	break;
    	}
    	}
    	if (queryUnderTest == null) {
    	org.junit.Assert.fail("Query JobExecution.getFirstCreatedByJobIdAndParamters does not exist anymore.");
    	}
    	String queryText = queryUnderTest.query();
    	// Minor changes with whitespace are ignored
    	queryText = queryText.trim().replace('\t', ' ').replace('\n', ' ').replace('\r', ' ');
    	...
    	org.junit.Assert.assertEquals("There's a change in the query string. Generated methods may not fit to the query anymore. Change from 'SELECT j FROM JobExecution j WHERE j.jobId = :jobId AND j.status = 'QUEUED' AND (j.parameters IS NULL OR j.parameters = :parameters) ORDER BY j.createdAt ASC' to '" + queryText + "'", "SELECT j FROM JobExecution j WHERE j.jobId = :jobId AND j.status = 'QUEUED' AND (j.parameters IS NULL OR j.parameters = :parameters) ORDER BY j.createdAt ASC", queryText);
    }

    /**
     * Tests that call and query are consistent for query 'JobExecution.getFirstCreatedByJobIdAndParamters' - no result.
     *
     */
    @org.junit.Test 
    public void testGetFirstCreatedByJobIdAndParamtersEmptyResult()
    {
    	Query query = org.mockito.Mockito.mock(Query.class);
    	EntityManager entityManager = org.mockito.Mockito.mock(EntityManager.class);
    	org.mockito.BDDMockito.given(entityManager.createNamedQuery("JobExecution.getFirstCreatedByJobIdAndParamters")).willReturn(query);
    	...
    	// Call
    	JobExecution result = io.coodoo.workhorse.jobengine.entity.JobExecution.getFirstCreatedByJobIdAndParamters(entityManager,jobId, parameters);
    	// Verification
    	org.mockito.BDDMockito.verify(entityManager, org.mockito.Mockito.times(1)).createNamedQuery("JobExecution.getFirstCreatedByJobIdAndParamters");
    ...
    /**
     * Tests that call and query are consistent for query 'JobExecution.getFirstCreatedByJobIdAndParamters' - one result.
     *
     */
    @SuppressWarnings({"unchecked", "rawtypes"})
    @org.junit.Test 
    public void testGetFirstCreatedByJobIdAndParamtersOneResult()
    {
    	Query query = org.mockito.Mockito.mock(Query.class);
    	EntityManager entityManager = org.mockito.Mockito.mock(EntityManager.class);
    	org.mockito.BDDMockito.given(entityManager.createNamedQuery("JobExecution.getFirstCreatedByJobIdAndParamters")).willReturn(query);
    	List results = new java.util.ArrayList();
    	JobExecution first = org.mockito.Mockito.mock(JobExecution.class);
    	JobExecution second = org.mockito.Mockito.mock(JobExecution.class);
    	results.add(first);
    	results.add(second);
    	org.mockito.BDDMockito.given(query.getResultList()).willReturn(results);
    	Long jobId = java.lang.Long.valueOf(0);
    ...
    public void testGetFirstCreatedByJobIdAndParamtersVerifyFields()
    {
    	...
    	if (field.getName().equals(fieldName)) {
    	if (fieldType != null && !field.getType().getName().equals(fieldType)) {
    	org.junit.Assert.fail("Error checking path " + fieldPath + " in query JobExecution.getFirstCreatedByJobIdAndParamters: The field " + clazz.getName() + "." + field + " does not have the type " + fieldType + " (anymore)");
    	}
    	fieldFound = true;
    	break;
    	}
    	...
    	if (!fieldFound) {
    	org.junit.Assert.fail("Error checking path " + fieldPath + " in query JobExecution.getFirstCreatedByJobIdAndParamters: The field " + className + "." + fieldName + " does not exist (anymore)");
    	}
    	}
    	} catch (ClassNotFoundException e) {
    	org.junit.Assert.fail("Error checking path " + fieldPath + " in query JobExecution.getFirstCreatedByJobIdAndParamters: The class "	+ className + " does not exist (anymore)");
    	}
    	}
    }

    /**
     * Tests that query 'JobExecution.countQueudByJobIdAndParamters' has not changed since this test had been created. If this test fails, you should consider re-generating ALL methods created from that query as they may be out-dated.
     *
     */
    @SuppressWarnings({"unchecked", "rawtypes", "null"})
    @org.junit.Test 
    public void testCountQueudByJobIdAndParamtersQueryUnchanged()
    {
    	...
    	org.junit.Assert.assertEquals("There's a change in the query string. Generated methods may not fit to the query anymore. Change from 'SELECT COUNT(j) FROM JobExecution j WHERE j.jobId = :jobId AND j.status = 'QUEUED' and (j.parameters IS NULL or j.parameters = :parameters)' to '" + queryText + "'", "SELECT COUNT(j) FROM JobExecution j WHERE j.jobId = :jobId AND j.status = 'QUEUED' and (j.parameters IS NULL or j.parameters = :parameters)", queryText);
    }
    ...
    public void testCountQueudByJobIdAndParamtersVerifyFields()
    {
    	...
    	if (field.getName().equals(fieldName)) {
    	if (fieldType != null && !field.getType().getName().equals(fieldType)) {
    	org.junit.Assert.fail("Error checking path " + fieldPath + " in query JobExecution.countQueudByJobIdAndParamters: The field " + clazz.getName() + "." + field + " does not have the type " + fieldType + " (anymore)");
    	}
    	fieldFound = true;
    	break;
    	}
    	...
    	if (!fieldFound) {
    	org.junit.Assert.fail("Error checking path " + fieldPath + " in query JobExecution.countQueudByJobIdAndParamters: The field " + className + "." + fieldName + " does not exist (anymore)");
    	}
    	}
    	} catch (ClassNotFoundException e) {
    	org.junit.Assert.fail("Error checking path " + fieldPath + " in query JobExecution.countQueudByJobIdAndParamters: The class "	+ className + " does not exist (anymore)");
    	}
    	}
    }
}
